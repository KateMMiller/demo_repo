#testTurbo.R tests the Turbo colorscale from Google
# Colorscale description https://ai.googleblog.com/2019/08/turbo-improved-rainbow-colormap-for.html?m=1
...
#Example with categorical data in a waffle plot
...
ggplot(df, aes(x = x, y = y, fill = category)) + 
  ...
  labs(title="Waffle Chart", subtitle="'Class' of vehicles",
       caption="Source: mpg") + 
  theme(#panel.border = element_rect(size = 2),  #AKB note: this gave me problems, so I commented it
...
# change color limits with pal.min and pal.max
ggplot(df, aes(x = x, y = y, fill = category)) + 
  geom_tile(color = "black", size = 0.5) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), trans = 'reverse') +   
  scale_fill_manual(values = Turbo(out.colors = 7,pal.min = .1,pal.max = .9)) +
  labs(title="Waffle Chart", subtitle="'Class' of vehicles",
       caption="Source: mpg") + 
  theme(#panel.border = element_rect(size = 2),  #AKB note: this gave me problems, so I commented it
    plot.title = element_text(size = rel(1.2)),
...
#Example with continuous data in a heatmap
...
#plot with partial color range (reds, oranges)
ggplot(df, aes(monthweek, weekdayf, fill = VIX.Close)) + 
  geom_tile(colour = "white") + 
  facet_grid(year~monthf) + 
  ###scale_fill_gradient(low="red", high="green") +##
  scale_fill_gradientn(colours = Turbo(pal.min = .6,pal.max = 1,reverse = T)) + 
  labs(x="Week of Month",
       y="",
       title = "Time-Series Calendar Heatmap", 
       subtitle="Yahoo Closing Price", 
...
#reverse is false, red is high
ggplot(df, aes(monthweek, weekdayf, fill = VIX.Close)) + 
  geom_tile(colour = "white") + 
  facet_grid(year~monthf) + 
  ###scale_fill_gradient(low="red", high="green") +##
  scale_fill_gradientn(colours = Turbo(pal.min = .6,pal.max = 1,reverse = F)) + 
  labs(x="Week of Month",
       y="",
       title = "Time-Series Calendar Heatmap", 
       subtitle="Yahoo Closing Price", 
...
df2 %>%
  ...
  geom_line()+
  scale_color_manual(values = Turbo(out.colors = length(df2)-1))
